...
import { Landmark, User as UserIcon, Lock, Phone, Contact, Eye, EyeOff } from "lucide-react";
...
export default function Register() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username.trim().includes(" ")) {
      showToast("Username should not contain spaces.", "warning");
      return;
    }
    ...
    setTimeout(() => {
      try {
        register(username.trim(), name.trim(), phone.trim(), password.trim());
      ...
      <div className="w-full max-w-md space-y-8 rounded-card border border-ink-100 bg-surface p-6 shadow-sm sm:p-8">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-1.5 text-center text-sm text-ink-500">
            Join the Tech Foxies Citizen Portal
          </p>
        </div>
