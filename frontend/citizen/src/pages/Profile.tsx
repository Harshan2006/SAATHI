...
import { Mail, MapPin, Phone, ShieldCheck, CalendarDays, LogOut, Loader2 } from "lucide-react";
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="rounded-card border border-ink-100 bg-surface p-5 shadow-sm">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-ink-900 text-xl font-semibold text-white uppercase">
            {initials}
          </div>
          <div>
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 border-t border-ink-100 pt-4 sm:grid-cols-2">
          <div className="flex items-center gap-2 text-sm text-ink-700">
            <Phone className="h-4 w-4 text-ink-400" /> {currentUser.phone || "No phone provided"}
          </div>
          <div className="flex items-center gap-2 text-sm text-ink-700">
            <Mail className="h-4 w-4 text-ink-400" /> {currentUser.email}
          </div>
          <div className="flex items-center gap-2 text-sm text-ink-700">
            <MapPin className="h-4 w-4 text-ink-400" /> {currentUser.location || "Jharkhand"}
          </div>
          <div className="flex items-center gap-2 text-sm text-ink-700">
            <CalendarDays className="h-4 w-4 text-ink-400" /> Joined {currentUser.joinedDate ? formatDate(currentUser.joinedDate) : "Recently"}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-card border border-ink-100 bg-surface p-5">
        ...
        <button
          disabled={isSaving}
          onClick={handleSave}
          className="mt-4 rounded bg-teal-600 px-4 py-2 text-sm font-medium text-white hover:bg-teal-700 disabled:opacity-50"
        >
