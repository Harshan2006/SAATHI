      ...
      <div className="grid grid-cols-2 gap-4 px-4 py-4 sm:grid-cols-4">
        ...
        <div>
          ...
          <select
            ...
            className="mt-0.5 rounded border border-ink-200 bg-surface px-2 py-1 text-sm font-medium text-ink-800 focus:border-teal-500 focus:outline-none"
          >
            {(["Low", "Medium", "High", "Critical"] as Priority[]).map((priority) => (
              <option key={priority} value={priority}>{priority}</option>
            ))}
          </select>
          <div className="mt-1"><PriorityBadge priority={analysis.priority} /></div>
        </div>
        <div>
