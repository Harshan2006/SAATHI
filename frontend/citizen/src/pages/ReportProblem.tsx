...
function titleFromDocument(text: string, filename: string) {
  const subject = text.match(/^\s*(?:subject|re)\s*:\s*(.+)$/im)?.[1].trim();
  if (subject) return subject.length > 96 ? `${subject.slice(0, 93).trimEnd()}…` : subject;
  const ignoredHeadings = /^(citizen complaint|complaint|report|description|subject|title)$/i;
  const firstMeaningfulLine = text.replace(/\r/g, "").split("\n")
    .map((line) => line.trim().replace(/^[#*\-\d.)\s]+/, ""))
    .find((line) => line.length > 12 && !ignoredHeadings.test(line));
  const source = firstMeaningfulLine || filename.replace(/\.[^.]+$/, "").replace(/[-_]+/g, " ");
...
export default function ReportProblem() {
  ...
  const handleDocumentUpload = async (file: File) => {
    ...
    try {
      ...
      // Update description with extracted text, keeping it editable
      setDescription(result.text);
      if (!title.trim()) {
        const suggestedTitle = titleFromDocument(result.text, file.name);
        if (suggestedTitle) setTitle(suggestedTitle);
      }
      showToast(`Content extracted from "${file.name}"!`, "success");
      
      // Optionally try to auto-detect category if possible, but for now just keep current
    } catch (error: any) {
      console.error("Extraction error:", error);
      showToast(error.message || "Failed to extract text from document.", "warning");
    } finally {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      const mappedAnalysis: AIAnalysis = {
        ...
        priority: normalizePriority(analysisData.analysis.severity),
  ...
  if (step === "analysis" && aiResult) {
    return (
      <div className="mx-auto max-w-2xl space-y-4">
        ...
        <AIAnalysisCard
          analysis={aiResult}
          onPriorityChange={(priority) => setAiResult((current) => current ? { ...current, priority } : current)}
          onContinue={async () => {
            if (backendProblemId) {
              try {
                await api.updateProblemPriority(backendProblemId, aiResult.priority);
              } catch (error) {
                console.error(error);
                showToast("Could not save the priority. Please try again.", "warning");
                return;
              }
            }
    ...
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <section className="space-y-4">
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-ink-500">A title is suggested from an uploaded document; you can edit it anytime.</p>
        </div>
      ...
      <section className="space-y-3">
        ...
        <UploadZone files={files} onAdd={addFiles} onRemove={removeFile} onParse={handleExtractedText} />
      </section>
