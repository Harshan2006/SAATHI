...
const TYPE_META: Record<AppNotification["type"], { icon: typeof CheckCircle2; accent: string }> = {
...
export default function NotificationPanel({ notification }: { notification: AppNotification }) {
  const meta = TYPE_META[notification.type];
  const Icon = meta.icon;

  return (
    <Link
      to={`/citizen/complaints/${notification.complaintId}`}
      className={cn(
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex items-center gap-2 text-xs text-ink-400">
          <span className="font-mono">{notification.complaintId}</span>
          <span>·</span>
